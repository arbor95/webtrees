...
final class Table extends DBALTable implements ComponentInterface
{
    public function __construct(string $name, ComponentInterface ...$components)
    {
        ...
        $columns        = array_map($column_mapper, array_filter($components, $column_filter));
        $foreign_keys   = array_map($foreign_key_mapper, array_filter($components, $foreign_key_filter));
        $indexes        = array_map($index_mapper, array_filter($components, $index_filter));
        $primary_keys   = array_map($primary_key_mapper, array_filter($components, $primary_key_filter));
        $unique_indexes = array_map($unique_index_mapper, array_filter($components, $unique_index_filter));
        ...
        // @todo - For MySQL, set ENGINE=InnoDB and ROW_FORMAT=DYNAMIC
    }
}
